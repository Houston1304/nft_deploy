...
(async () => {
    ...
    let collection = client4.open(NftCollection.fromAddress(Address.parse("EQDy9IPgtlBfZPQLKRSkuI4lVp1BPUpbNff4P-4rNKbQCXr8")));
    await collection.send(wallet.sender(pair.secretKey), {value: toNano("0.1")}, {$$type: 'RequestNftDeploy', index: 0n, amount: toNano("0.05"), content: createOffchainContent("https://raw.githubusercontent.com/Bquaith/ton_token/main/item.json"), owner: wallet.address})
})();
